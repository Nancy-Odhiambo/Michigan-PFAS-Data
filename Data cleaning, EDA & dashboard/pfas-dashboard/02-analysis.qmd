...
# Compute Michigan bounds for perfect fit
mi_bbox <- st_bbox(mi_counties)

# Define intuitive color palette: yellow (low) → red (high)
...
# Create popup labels with all variables plus type
...
# Build interactive Leaflet map with custom zoom and dual legends
leaflet(hazard_sites_full) |>
  # Center on Michigan with higher zoom
  ...
  # Hazard site markers
  addCircleMarkers(
    radius = ~sqrt(Hazard_Index) * 3,  # proportional to Hazard_Index
  ...
  # Numeric legend with styled title
  addLegend(
    position = "bottomleft",
    pal = hazard_pal,
    values = hazard_sites_full$Hazard_Index,
    title = HTML("<b style='color:#4B0082;'>Hazard Index</b>"),
    labFormat = labelFormat(
      transform = function(x) round(x, 2),   # numeric rounding only
      digits = 2
    )
  ) |>
  
  # Custom HTML control box for explanatory text
  addControl(
    html = "<div style='background:white;padding:10px;border-radius:5px;
                   box-shadow:0 0 5px rgba(0,0,0,0.3);'>
              <h4 style='margin:0;color:#4B0082;'>Hazard Guide</h4>
              <p style='margin:0;font-size:12px;'>
                Larger circles = high<br>
                Low (yellow) to High (red)
              </p>
            </div>",
    position = "topright"
  )
...
unique_values <- public_water_max_long |>
  ...
  mutate(analyte = ifelse(analyte %in% hazard_analytes, analyte, "Non-hazard analytes"),
...
p_mean <- ggplot(plot_data, aes(x = system_name, y = mean_value, fill = analyte,
  ...
  labs(title = "Mean PFAS Analyte Levels by Top 10 Systems",
       x = "System Name", y = "Mean Analyte Value (ppt)", fill = "Analyte",
       caption = "Data source: Michigan PFAS Action Response Team (MPART)") +
  theme_minimal() +
