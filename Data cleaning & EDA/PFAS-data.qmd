...
# Loading the Michigan counties shapefile
mi_counties <- st_read("GeoFiles/cb_2018_us_county_500k.shp") |>
  filter(STATEFP == "26") |>
  dplyr::select(GEOID, NAME)

# Computing the mean PFAS concentration by county
pfas_by_county <- surface_water_long |>
  group_by(geoid) |>
  summarize(mean_value = mean(analyte_value, na.rm = TRUE), .groups = "drop") |>
  left_join(mi_counties |> st_drop_geometry(), by = c("geoid" = "GEOID"))

# Selecting the top and bottom 15 Counties
pfas_top_bottom <- bind_rows(
  slice_max(pfas_by_county, order_by = mean_value, n = 15) |> mutate(group = "Top 15"),
  slice_min(pfas_by_county, order_by = mean_value, n = 15) |> mutate(group = "Bottom 15")
)

# Plotting the top and bottom 15 Counties PFAS concentration in surface water
ggplot(pfas_top_bottom, aes(x = fct_reorder(NAME, mean_value), y = mean_value, fill = group)) +
  geom_col() +
...
public_water_long |>
  ...
  summarize(total_concentration = sum(analyte_value, na.rm = TRUE), .groups = "drop") |>
  slice_max(total_concentration, n = 10) |>
  ggplot(aes(x = fct_reorder(analyte, total_concentration), 
             y = total_concentration, 
             fill = total_concentration)) +
  geom_col() +
  coord_flip() +
  scale_fill_viridis_c(option = "viridis") +
  labs(title = "Top 10 PFAS Analytes in Public Water",
       x = "Analyte", 
       y = "Total Concentration (ng/L)") +
  theme_minimal() +
  theme(legend.position = "bottom")
