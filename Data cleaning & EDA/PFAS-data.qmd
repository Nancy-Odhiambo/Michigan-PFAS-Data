...
pfas_county_summary <- surface_max_full |>
  group_by(geoid) |>
...
public_county_map <- surface_max_full |>
  group_by(geoid) |>
...
```{r}
# Calculating the Hazard index per sites_county
# Pivot analytes into columns
public_max_full_wide <- public_max_full |>
  pivot_wider(id_cols = c(geoid, longitude, latitude, system_name, system_type, sample_date, lab_name_code),
    names_from = analyte,
    values_from = analyte_value,
    values_fill = 0
  )

# Calculate Hazard_Index
public_max_full_wide <- public_max_full_wide |>
  mutate(Hazard_Index = (HFPODA / 10) + 
                   (PFNA / 10) +    
                   (PFBS / 2000) +  
                   (PFHxS / 10)     
  )
head(public_max_full_wide)
```

```{r}
# Plotting a bar graph of hazard index per site for the highest 15 counties
hazard_by_county <- public_max_full_wide |>
  group_by(geoid) |>
  summarize(
  ...
  ungroup() |>  
  left_join(
    mi_counties |> st_drop_geometry() |> select(GEOID, NAME),
    by = c("geoid" = "GEOID")
  ) |>
  filter(!is.na(NAME)) |>  
...
ggplot(hazard_by_county, aes(x = fct_reorder(NAME, total_hazard), y = total_hazard, fill = total_hazard)) +
  geom_col(show.legend = TRUE) +
  geom_text(aes(label = round(total_hazard, 2)),
            hjust = -0.1, size = 3.5, color = "black") +
  coord_flip() +
  scale_fill_viridis_c(option = "cividis", name = "Total Hazard Index") +
  labs(
    title = "Top 15 Michigan Counties by Total PFAS Hazard Index",
    subtitle = "Summed PFAS Hazard Index across all detected compounds",
    x = "County",
...
```{r}
# Top 15 Water Systems by Total Hazard Index
# Summarize total hazard index by system
top_systems <- public_max_full_wide |>
  group_by(system_name) |>
  summarize(
    total_hazard = sum(Hazard_Index, na.rm = TRUE),
    n_samples = n(),
  ...
  slice_head(n = 15) |>
  ungroup() |>
  mutate(system_name = fct_reorder(system_name, total_hazard))

ggplot(top_systems, aes(x = system_name, y = total_hazard, fill = total_hazard)) +
  geom_col() +
  geom_text(aes(label = round(total_hazard, 2)),
            ...
            color = "grey") +
  coord_flip() +
  scale_fill_viridis_c(option = "plasma", name = "Total Hazard Index") +
  labs(
    title = "Top 15 Water Systems by Total PFAS Hazard Index",
    subtitle = "Summed PFAS Hazard Index across all detected compounds",
    x = "Water System",
    y = "Total Hazard Index",
    caption = "Data Source: Michigan PFAS Action Response Team (MPART), EGLE"
...
```{r}
# Summarize total hazard by system type (all, descending order)
system_type_table <- public_max_full_wide |>
...
# Create flextable without highlighting
flextable(system_type_table) |>
  set_header_labels(
    system_type = "System Type",
    total_hazard = "Total Hazard Index",
...
```{r}
# Plot showing the PFAS sites by their level of Hazard Index
# Ensure PFAS sites has geoid and coordinates
pfas_sites_sf <- st_as_sf(pfas_sites, coords = c("longitude", "latitude"), crs = 4326)

# Joining data sets
pfas_sites_hazard <- pfas_sites_sf |>
  left_join(public_max_full_wide |>
              select(geoid, Hazard_Index), by = "geoid")

# Plot sites colored by hazard index
ggplot() +
  geom_sf(data = mi_counties, fill = "grey", color = "mediumseagreen") +
  geom_sf(data = pfas_sites_hazard, aes(color = Hazard_Index), size = 2) +
  scale_color_viridis_c(option = "plasma", trans = "sqrt") +
  labs(title = "PFAS Sites in Michigan with Hazard Index",
       color = "Hazard Index",
       caption = "Data source: Michigan PFAS Action Response Team (MPART), EGLE") +
  theme_minimal()
```
